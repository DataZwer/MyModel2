...
if __name__ == '__main__':
    ...
    iterations, next_iterator = dataset_iterator(test_sens_pad, test_labels_pad)
    with tf.Session() as sess:
        for epoch in range(1):
            for iteration in range(iterations):
                sen_batche, labels_batch = sess.run(next_iterator)
                break
            break
